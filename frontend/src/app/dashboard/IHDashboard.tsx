      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <div className="card">
          <div className="card-body">
            <h3 className="text-sm font-medium text-slate-500">Papers awaiting your approval</h3>
            <p className="mt-1 text-3xl font-semibold text-slate-900">{papersAwaitingApproval.length}</p>
          </div>
        </div>
        <div className="card">
          <div className="card-body">
            <h3 className="text-sm font-medium text-slate-500">Overdue escalated</h3>
            <p className="mt-1 text-3xl font-semibold text-slate-900">{overdueEscalatedCount}</p>
          </div>
        </div>
        <div className="card">
          <div className="card-body">
            <h3 className="text-sm font-medium text-slate-500">Pending inter-ministerial</h3>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card">
          <div className="card-header flex items-center justify-between">
            ...
            <Link href="/reports" className="text-sm font-medium text-blue-600 hover:underline" title="Reports / approval pipeline">
              View all
            </Link>
          </div>
          <div className="card-body">
            {papersAwaitingApproval.length === 0 ? (
              <p className="text-sm text-slate-500">No papers currently awaiting your approval.</p>
            ) : (
              <ul className="space-y-2">
                {papersAwaitingApproval.map((p) => (
                  <li key={p.id} className="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2">
                    ...
                    <span className="text-xs text-slate-500">{p.agingDays}d</span>
                  </li>
                ))}
              </ul>
            )}
            <p className="mt-2 text-xs text-slate-400">Approval workflow (SCR-PAPER-03) — full list when approval service is connected.</p>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="card-body">
            {upcomingMeetings.length === 0 ? (
              <p className="text-sm text-slate-500">No upcoming meetings in the next period.</p>
            ) : (
              <ul className="space-y-2">
                {upcomingMeetings.slice(0, 5).map((m) => (
                  <li key={m.meetingId}>
                    ...
                    >
                      ...
                      <span className="text-xs text-slate-500">{new Date(m.startDate).toLocaleDateString()}</span>
                    </Link>
                  </li>
                ))}
              </ul>
        ...
        <div className="card">
          ...
          <div className="card-body">
            {participationSummary.length === 0 ? (
              <p className="text-sm text-slate-500">No committee participation data yet.</p>
            ) : (
              <ul className="space-y-2">
                {participationSummary.map((s) => (
                  <li key={s.committeeName} className="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2">
                    ...
                    <span className="text-sm text-slate-500">{s.meetingsCount} meetings</span>
                  </li>
                ))}
              </ul>
            )}
            <p className="mt-2 text-xs text-slate-400">High-level view across all active committees (reporting service when connected).</p>
          </div>
        </div>
