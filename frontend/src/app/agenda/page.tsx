...
async function getMeetings(accessToken: string): Promise<MeetingDto[]> {
  const res = await fetch(`${getApiUrl()}/api/v1/meetings?size=50&status=ACTIVE`, {
  ...
  const planned = await fetch(`${getApiUrl()}/api/v1/meetings?size=50&status=PLANNED`, {
    headers: { Authorization: `Bearer ${accessToken}` },
    cache: 'no-store',
  }).then((r) => (r.ok ? r.json() : { content: [] })).then((d) => d.content ?? []);
  const combined = [...list, ...(Array.isArray(planned) ? planned : [])];
  const seen = new Set<string>();
  return combined.filter((m: MeetingDto) => {
    if (seen.has(m.meetingId)) return false;
    seen.add(m.meetingId);
    return true;
  }).sort((a: MeetingDto, b: MeetingDto) => (a.startDate ?? '').localeCompare(b.startDate ?? ''));
}
    ...
    <div>
      ...
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Agenda items are managed per meeting. Open a meeting to view, add, or edit agenda items.
        </p>
      </div>
      <div className="card mb-6">
        ...
        <div className="card-body">
          <form method="get" action={`${basePath}/agenda`} className="flex flex-wrap gap-4 items-end">
                  ...
                  {meetings.map((m) => (
                    <tr key={m.meetingId} className="hover:bg-slate-50/50">
                      ...
                      <td className="table-cell text-right">
                        <Link href={`/meetings/${m.meetingId}?tab=agenda`} className="text-blue-600 hover:underline">
                          Open agenda →
                        </Link>
                      </td>
                    </tr>
