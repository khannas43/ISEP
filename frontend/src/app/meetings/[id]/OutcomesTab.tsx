  ...
  return (
    <div className="card">
      <div className="card-header flex flex-wrap items-center justify-between gap-3">
        ...
        {canEdit && (
          <Link href={`/meetings/${meetingId}/outcomes`} className="btn-secondary text-sm">
          ...
          <ul className="space-y-6">
            {outcomes.map((o) => (
              <li key={o.outcomeId} className="rounded-lg border border-slate-200 bg-slate-50/50 p-4">
                ...
                {o.resolutionRef && (
                  <p className="mt-1 text-sm text-slate-600">Resolution: {o.resolutionRef}</p>
                )}
                {o.nextSteps && (
                  <p className="mt-1 text-sm text-slate-600">Next steps: {o.nextSteps}</p>
                )}
                <p className="mt-2 text-xs text-slate-500">Captured {formatDate(o.capturedAt)}</p>
              </li>
            ))}
          </ul>
        )}
