      ...
      <div className="flex min-h-[50vh] flex-1 flex-col border-slate-200 lg:border-r">
        ...
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-2 sm:px-6">
          ...
          <select
            ...
            className="w-full max-w-xl rounded-lg border border-slate-300 bg-white px-3 py-2 text-base"
            value={selectedItem ?? ''}
        ...
        >
          {loadError && <p className="text-center text-base text-amber-700">{loadError}</p>}
          {posts.length === 0 && !loadError && (
            <p className="py-12 text-center text-base text-slate-400">No posts yet. Be the first to contribute.</p>
          )}
          {posts.map((post, i) => (
            <div
              key={post.postId || `p-${i}`}
            ...
            >
              <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2">
                <span className="text-base font-semibold text-[var(--navy-700)]">{post.postedByName}</span>
              ...
              <p className="m-0 text-base leading-relaxed text-slate-700">{post.content}</p>
            </div>
          ))}
        </div>

        <div className="border-t border-slate-200 bg-white px-4 py-4 sm:px-6">
          {postError && <p className="mb-2 text-base text-red-600">{postError}</p>}
          <div className="mb-2 flex flex-wrap gap-2">
            {(['COMMENT', 'INTERVENTION', 'INFORMATION', 'POINT_OF_ORDER'] as const).map((type) => (
              <button
                ...
                className={`rounded border px-2.5 py-1 text-[14px] font-medium ${
          ...
          <div className="flex gap-2">
            <textarea
              ...
              rows={2}
              className="min-h-[3rem] flex-1 resize-none rounded-lg border border-slate-200 px-3 py-2 text-base outline-none focus:border-[var(--navy-400)]"
              onKeyDown={(e) => {
            ...
            <button
              ...
              className="self-stretch rounded-lg bg-[var(--navy-600)] px-5 text-base font-semibold text-white disabled:opacity-50"
            >
              Post
            </button>
          </div>
          <p className="mt-1 text-[14px] text-slate-500">⌘+Enter to post quickly</p>
        </div>
      </div>
      ...
      <aside className="w-full shrink-0 border-t border-slate-200 bg-white p-5 lg:w-[280px] lg:border-t-0">
        ...
        <RoleGuard allowedRoles={['SYSTEM_ADMIN', 'DELEGATION_LEADER']}>
          <button
            ...
            disabled={isLive}
            className="mb-2.5 w-full rounded-lg bg-[var(--navy-600)] py-2.5 text-base font-semibold text-white disabled:cursor-default disabled:bg-slate-200 disabled:text-slate-500"
          >
          ...
          <button
            ...
            className="w-full rounded-lg border border-red-300 py-2.5 text-base font-semibold text-red-700"
          >
            Lock discussion
          </button>
        </RoleGuard>
        ...
        <Link href={`/meetings/${meetingId}?tab=live`} className="mt-6 inline-block text-base text-slate-600 hover:text-slate-900">
          ← Meeting overview
        </Link>
      </aside>
    </div>
