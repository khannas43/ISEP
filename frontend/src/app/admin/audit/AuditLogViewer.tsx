      ...
      <div className="card">
        <div className="card-body">
          <div className="flex flex-wrap gap-4">
            <div>
              <label htmlFor="audit-user-filter" className="block text-xs font-medium text-slate-500">User</label>
            ...
            <div>
              <label htmlFor="audit-action-filter" className="block text-xs font-medium text-slate-500">Action type</label>
            ...
            <div className="flex items-end gap-2">
              <button type="button" className="btn-secondary text-sm">Export CSV</button>
              <button type="button" className="btn-secondary text-sm">Export JSON</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
            ...
            <tbody className="divide-y divide-slate-200">
              {pageEntries.map((e) => (
                <Fragment key={e.id}>
                  ...
                  {expandedId === e.id && (
                    <tr>
                      <td colSpan={6} className="table-cell bg-slate-50 px-4 py-2 font-mono text-xs">
        ...
        <div className="flex items-center justify-between border-t border-slate-200 px-4 py-2 text-sm text-slate-600">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0} className="btn-secondary text-sm disabled:opacity-50">Previous</button>
            <button type="button" onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="btn-secondary text-sm disabled:opacity-50">Next</button>
          </div>
        </div>
      </div>
    </div>
