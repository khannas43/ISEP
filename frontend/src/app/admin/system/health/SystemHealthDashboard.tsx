...
export function SystemHealthDashboard({ services }: Props) {
  return (
    <div className="space-y-4">
      <p className="text-base text-slate-500">Last updated: {new Date().toLocaleString()}. Refresh every 30s in production.</p>
          ...
          <div key={s.id} className="card">
            <div className="card-body flex flex-row items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Last heartbeat: {new Date(s.lastHeartbeat).toLocaleTimeString()}</p>
                <p className="mt-1 text-base text-slate-600">Response: {s.responseTimeMs} ms · Error rate: {(s.errorRate * 100).toFixed(2)}%</p>
              </div>
