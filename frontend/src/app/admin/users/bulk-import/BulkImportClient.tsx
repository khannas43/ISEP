    ...
    <div className="card">
      <div className="card-body space-y-6">
        <form onSubmit={handleValidate} className="space-y-4">
          <div>
            <label htmlFor="csv-file" className="block text-base font-medium text-slate-700">CSV file</label>
            <input
              id="csv-file"
              type="file"
              accept=".csv,text/csv"
              onChange={(e) => { setFile(e.target.files?.[0] ?? null); setResult(null); setCreated(null); }}
              className="mt-1 block w-full max-w-md text-base text-slate-600 file:mr-4 file:rounded file:border-0 file:bg-slate-100 file:px-4 file:py-2 file:text-base file:font-medium file:text-slate-700 hover:file:bg-slate-200"
            />
          </div>
        ...
        {error && <p className="rounded bg-red-50 px-4 py-2 text-base text-red-700" role="alert">{error}</p>}
        {created !== null && (
          <p className="rounded bg-emerald-50 px-4 py-2 text-base text-emerald-700">
        ...
        {result && (
          <div className="space-y-4">
            <h2 className="text-base font-semibold text-slate-700">Validation result</h2>
            {result.valid.length > 0 && (
              <div>
                <p className="text-base text-slate-600">{result.valid.length} valid row(s)</p>
            ...
            {result.invalid.length > 0 && (
              <div>
                <p className="text-base text-amber-700">{result.invalid.length} invalid row(s)</p>
            ...
            {result.valid.length === 0 && result.invalid.length === 0 && (
              <p className="text-base text-slate-500">No data rows in file. Add a header row (email, fullName, …) or data rows.</p>
            )}
          </div>
        )}
      </div>
