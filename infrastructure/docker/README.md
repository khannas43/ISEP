...
**Port conflicts (common on a shared laptop):** If **9200**, **5601**, or **8081** are already in use, copy `infrastructure/docker/.env.example` to `infrastructure/docker/.env` and adjust values. Compose loads that `.env` automatically (project directory = folder with the compose file).
...
```bash
# From repo root
docker compose -f infrastructure/docker/docker-compose.dev.yml up -d --remove-orphans
```

If you **do not** use a `.env` file, pass variables on the command line when needed, e.g.  
`OPENSEARCH_HOST_PORT=9201 OPENSEARCH_DASHBOARDS_HOST_PORT=5602 MEETING_SERVICE_HOST_PORT=8082 docker compose -f infrastructure/docker/docker-compose.dev.yml up -d`
